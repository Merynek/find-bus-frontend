...
const CreateTripPage = observer(() => {
    ...
    return <div className={styles.layout}>
        <div className={styles.section}>
            ...
            <NumberBox
                value={_storeRef.current.trip.handicappedUserCount}
                onChange={(val) => {
                    if (val) {
                        _storeRef.current.trip.handicappedUserCount = val;
                    }
                }}
                minValue={0}
            />
        ...
        <div className={styles.section}>
            ...
            <DatePicker
                selected={_storeRef.current.trip.endOrder}
                onChange={(val) => {
                    if (val) {
                        _storeRef.current.trip.endOrder = val;
                    }
                }}
                placeholderText={"End"}
                showTimeSelect={true}
        ...
        <div>
            {!_storeRef.current.endOrderIsValid && <p style={{color: "red"}}>Min EndOrder From Now is {cfg.minEndOrderFromNowInHours} in Hours</p>}
            {!_storeRef.current.endOrderWithStartTripIsValid && <p style={{color: "red"}}>Min diff between EndOrder and StartTrip is {cfg.minDiffBetweenStartTripAndEndOrderInHours} in Hours</p>}
        </div>
        <div className={styles.section}>
            <span>Pocet osob: </span>
            <NumberBox
                value={_storeRef.current.trip.numberOfPersons}
                onChange={(val) => {
                    if (val) {
                        _storeRef.current.trip.numberOfPersons = val;
                    }
                }}
                minValue={0}
            />
        ...
        <div className={styles.section}>
            ...
            <CheckBox
                value={_storeRef.current.trip.dietForTransporter}
                onChange={(val) => _storeRef.current.trip.dietForTransporter = val}
        ...
        <div>
            {Object.values(Amenities).map((amenity, index) => {
                return <CheckBox
                    key={index}
                    label={amenity}
                    value={_storeRef.current.trip.amenities.indexOf(amenity) > -1}
                    onChange={(val) => {
                        if (val) {
                            _storeRef.current.trip.amenities.push(amenity);
                        } else {
                            const index = _storeRef.current.trip.amenities.indexOf(amenity);
                            removeOnIndex(_storeRef.current.trip.amenities, index);
                        }
                    }}
                    size={CheckBoxSize.MEDIUM}
                />
        ...
        <div className={styles.trips}>
            <div className={styles.stopsSection}>
                {_storeRef.current.trip.routes.map((route, index) => {
                    return <div key={index}>
                        <TripRouteCreate
                            route={route}
                            trip={_storeRef.current.trip}
                        />
                        <ButtonClick
                            ...
                            onClick={() => {
                                removeOnIndex(_storeRef.current.trip.routes, index);
                            }}
                        />
                    </div>
                })}
            </div>
            {_storeRef.current.displayRecommendation && <div className={cn(styles.stopsSection, styles.recommendation)}>
                <p><b>ECONOMY</b> version for one driver</p>
                {_storeRef.current.tripRecommendation === TripRecommendationType.ONE_DRIVER && <>{_storeRef.current.trip.routes.map((route, index) => {
                    return <div key={index}>
                        <TripRouteRecommendation
                            route={route}
                            trip={_storeRef.current.trip}
                ...
                {_storeRef.current.tripRecommendation === TripRecommendationType.TWO_DRIVERS && <div>
                    <p>REDUCE ROUTE: {_storeRef.current.reduceRoutesHours} Hours</p>
                    <p>REDUCE TIME: {_storeRef.current.reduceTimeHours} Hours</p>
                </div>}
            </div>}
        </div>
        <LayoutFlexColumn gap={FlexGap.MEDIUM_24}>
            <ButtonClick
                ...
                type={ButtonType.YELLOW}
                onClick={() => {
                    _storeRef.current.trip.addRoute();
                }}
            />
            <ButtonClick
                size={ButtonSize.BY_CONTENT}
                ...
                onClick={async () => {
                    let errors = "";
                    _storeRef.current.validate();
                    if (!_storeRef.current.isValid) {
                        if (!_storeRef.current.placesAreSet) {
                            errors += " @ All places are required, "
                        }
                        if (!_storeRef.current.peopleCountIsValid) {
                            errors += " @ Minimum count of people is 1, "
                        }
                        if (!_storeRef.current.routesCountIsValid) {
                            errors += " @ Minimum count of routes is 1"
                        }
                        if (!_storeRef.current.userSettings?.isValidForCreateInvoice) {
        ...
        <div style={{position: "relative", width: "100%", height: "300px"}}>
            <DirectionsMap
                directions={_storeRef.current.trip.directions}
                markers={_storeRef.current.trip.markers}
                center={_storeRef.current.trip.stops.map(s => s.place.point).filter<GeoPoint>((p) : p is GeoPoint => p !== undefined)}
                initialView={{
                    zoom: 11
                }}
            />
